<template>
    <main class="flex-1 relative overflow-y-auto py-6 focus:outline-none" tabindex="0">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
                <div class="md:flex md:items-center md:justify-between">
                    <div class="flex-1 min-w-0">
                        <h1 class="py-0.5 text-2xl font-semibold text-gray-900">Edit HTML Snippet</h1>
                    </div>
                    <div class="mt-4 flex md:mt-0 md:ml-4">
                        <button
                            class="btn btn-red shadow-sm rounded-md"
                            type="button"
                            @click="deleteSnippetModal = true"
                        >
                            Delete snippet
                        </button>
                    </div>
        ...
        <div v-show="deleteSnippetModal" class="fixed z-20 inset-0 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">

                <div v-show="deleteSnippetModal" class="fixed inset-0 transition-opacity">
                    <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
                </div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen"></span>&#8203;

                <div
                    v-show="deleteSnippetModal"
                ...
                >
                    ...
                    <div class="bg-gray-100 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        ...
                        <button
                            ...
                            type="button"
                            @click="deleteSnippetModal = false"
                        >
                            Cancel
                        </button>
                    </div>
...
export default {
    ...
    methods: {
        ...
        saveSnippet() {
            ...
            axios.put('/api/admin/snippets/' + self.$route.params.id, self.snippet).then(function (response) {
        ...
        getSnippet() {
            const self = this;
            self.loading = true;
            axios.get('/api/admin/snippets/' + self.$route.params.id).then(function (response) {
        ...
        deleteSnippet() {
            const self = this;
            axios.delete('/api/admin/snippets/' + self.$route.params.id).then(function () {

                self.$router.push('/admin/snippets/');
            }).catch(function () {
                self.deleteSnippetModal = false;
            });
        },
    }
}
